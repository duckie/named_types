...
template <class T>
builder{template <class... U> T* operator()(U && ... args){
...
template <size_t EncStr>
using attr = named_tag<typename decodestr<EncStr1>::type>;

template <class... T> struct factory;

template <class... Types, class... Tags>
struct factory<Types(Tags)...>
    : public const_rt_view<base_const_rt_view, named_tuple<Types(Tags)...>> {};
}

int main() {
  auto test = make_named_tuple(at<"name"_h>() = std::string("Roger"),
                               at<"age"_h>() = 47,
                               at<"size"_h>() = 1.92,
                               at<"list"_h>() = std::vector<int>{1, 2, 3});

  std::cout << at<"name"_h>(test) << "\n" << at<"age"_h>(test) << "\n"
