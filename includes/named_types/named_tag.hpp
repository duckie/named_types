...
template <class Spec, class Arg> struct __ntag_notation<Arg(Spec)> {
  using type = Spec(Arg);
};
...
// Unfortunately, MSVC doest not implement expression SFINAE
 ...
 public:
  static constexpr bool value =
      std::is_same<decltype(test<T>(0)), char const*>::value;
};

template <class T> class constexpr_type_name {
  template <class TT>
  static inline constexpr auto extract(int) -> decltype(TT::classname) {
    return TT::classname;
  }
// Unfortunately, MSVC doest not implement expression SFINAE
#ifndef _MSC_VER
  template <class TT>
  static inline constexpr auto extract(int) -> decltype(TT::name) {
    return TT::name;
  }
  template <class TT>
  static inline constexpr auto extract(int) -> decltype(TT::classname()) {
...
template <class T>
char const*
    constexpr_type_name<T>::value = constexpr_type_name<T>::extract<T>();

//// Name extractors specified to work with string literals
template <class T, T... chars>
class has_user_defined_name<string_literal<T, chars...>> {
 public:
...
template <class Tag> struct named_tag : std::tag::basic_tag {
 private:
  ...
  template <typename T> struct unnested_<named_tag<T>> {
    using type = named_tag<T>;
  };
  template <typename T> struct unnested_value_ { using type = T; };
  template <typename T> struct unnested_value_<named_tag<T>> {
    using type = T;
  };
 ...
 public:
  ...
  template <typename T>
  __attribute_const_reference_holder<value_type, T> operator=(T const& value) {
    return __attribute_const_reference_holder<value_type, T>(value);
  }

  template <typename T>
  __attribute_reference_holder<value_type, T> operator=(T& value) {
    return __attribute_reference_holder<value_type, T>(value);
  }

  template <typename T>
  __attribute_value_holder<value_type, T> operator=(T&& value) {
...
template <class Tag, typename Value> class __attribute_const_reference_holder {
 ...
 public:
  ...
  __attribute_const_reference_holder(
      __attribute_const_reference_holder const&) = delete;
  __attribute_const_reference_holder(__attribute_const_reference_holder&&) =
      default;
  __attribute_const_reference_holder&
  operator=(__attribute_const_reference_holder const&) = delete;
  __attribute_const_reference_holder&
  operator=(__attribute_const_reference_holder&&) = default;
...
template <class Tag, typename Value> class __attribute_reference_holder {
 ...
 public:
  ...
  __attribute_reference_holder&
  operator=(__attribute_reference_holder const&) = delete;
  __attribute_reference_holder&
  operator=(__attribute_reference_holder&&) = default;
...
template <class Tag, class Tagged> decltype(auto) get(Tagged&& input) {
