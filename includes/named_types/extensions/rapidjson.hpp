...
template <class RootType, class Encoding> class reader_handler : public ::rapidjson::BaseReaderHandler<Encoding, reader_handler<RootType,Encoding>> {
  static_assert(is_sub_object<RootType>::value || is_sequence_container<RootType>::value, "Root type of a handler must either be a named_tuple, an AssociativeContainer or a SequenceContainer.");
  ...
  struct Node {
    ...
    std::unique_ptr<parsing::sequence_pusher_interface<Ch,Ch,SizeType>> array_node;

    Node(parsing::value_setter_interface<Ch,Ch,SizeType>* obj_interface, parsing::sequence_pusher_interface<Ch,Ch,SizeType>* array_interface)
