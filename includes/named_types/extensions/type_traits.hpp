...
template <class T> struct is_sub_object {
  static constexpr bool const value = is_named_tuple<T>::value || is_associative_container<T>::value;
};

template <class T> struct is_sub_element {
  static constexpr bool const value = is_named_tuple<T>::value || is_sequence_container<T>::value || is_associative_container<T>::value;
};
